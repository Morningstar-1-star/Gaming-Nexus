...
const state = {
  tab: 'home',
  games: {
    newReleases: [], upcoming: [], trending: [], top: [],
    action: [], rpg: [], horror: [], shooter: [],
    indie: [], adventure: [], strategy: [],
  },
  loading: true,
  activeGame: null,
...
// ===========================================================================
// HOME — new releases hero + category shelves
// ===========================================================================
function renderHome() {
  const wrap = h('div', { class: 'fade-up', style: { paddingBottom: '80px' } });

  // header
  wrap.append(h('div', { style: { padding: '52px 16px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' } },
    h('div', null,
      h('p', { style: { color: 'var(--muted)', fontSize: '13px' } }, 'Discover'),
      h('h1', { style: { fontSize: '22px', fontWeight: 800, letterSpacing: '-0.5px', marginTop: '2px' } }, "What's New"),
    ),
    h('button', {
      class: 'tap',
      style: { width: '38px', height: '38px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' },
      onClick: () => { vib(); state.showSteam = true; render(); },
    }, I('gamepad', 17, { style: 'color:var(--muted)' })),
  ));

  if (state.loading) {
    wrap.append(h('div', { style: { padding: '0 16px' } },
      h('div', { class: 'skeleton', style: { height: '200px', borderRadius: '18px', marginBottom: '24px' } }),
    ));
    for (let i = 0; i < 4; i++) {
      wrap.append(h('div', { style: { marginBottom: '24px' } },
        h('div', { class: 'skeleton', style: { height: '13px', width: '130px', marginBottom: '12px', marginLeft: '16px', borderRadius: '6px' } }),
        h('div', { style: { display: 'flex', gap: '10px', padding: '0 16px', overflow: 'hidden' } },
          ...[0,1,2,3].map(() => h('div', { style: { flexShrink: 0 } },
            h('div', { class: 'skeleton', style: { width: '130px', height: '90px', borderRadius: '12px', marginBottom: '6px' } }),
            h('div', { class: 'skeleton', style: { width: '100px', height: '10px', borderRadius: '5px' } }),
          ))
        ),
      ));
    }
  ...
  // ---- HERO: new releases carousel ----
  const newGames = state.games.newReleases;
  if (newGames.length) {
    const carousel = h('div', {
      class: 'snap-x hide-scroll',
      style: { display: 'flex', gap: '12px', padding: '0 16px 4px', overflowX: 'auto' },
    });
    newGames.slice(0, 8).forEach(game => {
      carousel.append(h('div', {
        class: 'hero-card snap-start tap no-select',
        style: { minWidth: '78vw', maxWidth: '310px', flexShrink: 0 },
        ...
        h('div', { style: { position: 'absolute', top: '10px', left: '10px', background: 'var(--accent)', color: 'white', fontSize: '9px', fontWeight: 800, padding: '3px 8px', borderRadius: '6px', letterSpacing: '0.05em' } }, 'NEW'),
        h('div', { class: 'hero-meta' },
          h('p', { class: 'line-clamp-1', style: { fontSize: '15px', fontWeight: 700, marginBottom: '3px' } }, game.name),
          h('div', { style: { display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' } },
            ...
            game.released ? h('span', { style: { fontSize: '11px', color: 'rgba(255,255,255,0.4)' } }, game.released.slice(0, 7)) : null,
          ),
          h('div', { style: { display: 'flex', gap: '8px' } },
            ...
            }, I('play', 12, { style: 'color:#000' }), 'Watch'),
            h('button', {
              class: 'tap',
              style: { padding: '8px 12px', background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: '10px', fontSize: '12px', fontWeight: 600, color: 'white' },
              onClick: (e) => {
  ...
  const SHELVES = [
    ...
    { key: 'action',    title: '\u2694\uFE0F Action'         },
    { key: 'rpg',       title: '\uD83E\uDDD9 RPG'            },
    { key: 'horror',    title: '\uD83D\uDC7B Horror'         },
    { key: 'shooter',   title: '\uD83C\uDFAF Shooter'        },
    { key: 'indie',     title: '\uD83D\uDC8E Indie'          },
    { key: 'adventure', title: '\uD83D\uDDFA Adventure'      },
    { key: 'strategy',  title: '\u265F\uFE0F Strategy'       },
  ];

  SHELVES.forEach(({ key, title }) => {
    const games = state.games[key] || [];
    if (!games.length) return;
    wrap.append(renderShelf(title, games));
  });

  return wrap;
}
function renderShelf(title, data, ranked = false) {
...
// ===========================================================================
// BOOT — loads new releases + all genre shelves in parallel
// ===========================================================================
async function boot() {
  // Phase 1: load critical data first, show skeleton until done
  try {
    const [newReleases, upcoming, trending, top] = await Promise.all([
      API.newReleases(),
      API.upcomingGames(),
      API.trendingGames(),
      API.topGames(),
    ]);
    state.games.newReleases = newReleases;
    state.games.upcoming    = upcoming;
    state.games.trending    = trending;
    state.games.top         = top;
  } catch (e) {
  ...
  // Phase 2: load genre shelves in background after UI is visible
  const GENRE_SLUGS = {
    action:    'action',
  ...
  const entries = Object.entries(GENRE_SLUGS);
  // Load 2 at a time to avoid rate limiting
  for (let i = 0; i < entries.length; i += 2) {
    const batch = entries.slice(i, i + 2);
    await Promise.all(batch.map(async ([key, slug]) => {
      try {
        state.games[key] = await API.genreGames(slug);
      } catch {}
    }));
    render(); // re-render after each batch so shelves appear progressively
    await new Promise(r => setTimeout(r, 300)); // small gap between batches
  }
}
